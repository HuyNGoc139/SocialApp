...
import { View, Text, Image, Alert, TextInput, Button, TouchableOpacity } from 'react-native';
...
const MessageItem: React.FC<MessageItemProps> = ({ mess, currenUser }) => {
    ...
    const handleSaveMessage = () => {
        ...
        messageRef.update({
            text: editedText,
            updatedAt: firestore.FieldValue.serverTimestamp(),  // Thêm thời gian cập nhật
        })
        .then(() => {
            setIsEditing(false);  // Quay lại chế độ xem bình thường sau khi lưu
            ...
            Alert.alert(
                ...
                [
                    { text: "Cancel", style: "cancel" },
                    {
                        text: "OK",
                        onPress: async () => {
                            const messageRef = firestore().collection('Rooms').doc(mess.roomId).collection('messages').doc(mess.id);
    ...
    if(currenUser.uid==mess.userId){
        return(
            ...
            {isEditing && (
                        <TouchableOpacity style={{paddingBottom:12}} onPress={handleSaveMessage}>
