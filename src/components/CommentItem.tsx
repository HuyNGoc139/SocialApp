...
const CommentItem = (props:any) => {
                ...
                [
                    { text: "Cancel", style: "cancel" },
                    {
                        ...
                        onPress: async () => {
                            ...
                            const commentRef = postRef.collection('comments').doc(cmt.commentId); // Tham chiếu đến document của comment

                            await commentRef.delete(); // Xóa comment
    ...
    return(
        <View style={styles.container}>
            {cmt?.profileUrl?<Image style={{ height: 42, width: 42, borderRadius: 12, marginRight: 12 }} source={{uri:cmt.profileUrl}} /> 
            ...
            <Text>
                {handleDateTime.GetHour(cmt.createdAt)}
            </Text>
            </View>
            <View>
            <Text style={{fontFamily:fontFamilies.regular,
            fontSize:14}}>{cmt.text}</Text>
            </View>
            </View>
            {cmt.userId==userCurrent.uid||cmt.userId==userCurrent.userId?<TouchableOpacity onPress={handelDeleteComment}>
